...
tf.cg_posy = [200, 388]; // サムネイルのY座標
tf.cg_thumbnail_width  = 320; // サムネイルの幅
tf.cg_thumbnail_height = 180; // サムネイルの高さ

tf.btn_prev_pos = [56, 340]; // 前ページに遷移するボタンの座標
tf.btn_next_pos = [1168, 340]; // 次ページに遷移するボタンの座標

[endscript]

; ページネーション（ページ数に合わせてtextの「/3」を修正する）
[macro name="pagination"]
  [layopt layer="0" visible="true"]
  [free layer="0" name="pagination" time="1"]
  [ptext layer="0" name="pagination" text="&tf.page + 1 + '/3'" x="0" y="620" size="18" color="0x594756" bold="bold" width="1280" align="center"]
  [endmacro]
...
; ギャラリーモード終了
[button graphic="append_theme/gallery_close.png" enterimg="append_theme/gallery_close2.png" target="*backtitle" x="1218" y="32"]
...
; 次ページへ
[button graphic=append_theme/gallery_next.png enterimg=append_theme/gallery_next2.png target=*nextpage x="&tf.btn_next_pos[0]" y="&tf.btn_next_pos[1]"]
...
; 前のページへ
[button graphic=append_theme/gallery_prev.png enterimg=append_theme/gallery_prev2.png target=*prevpage x="&tf.btn_prev_pos[0]" y="&tf.btn_prev_pos[1]"]
